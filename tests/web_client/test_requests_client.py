...
class TestRequestsClient(TestCase):
    TEST_BASE_URL = 'http://test-awesome-sitemap-parser.com'  # mocked by HTTPretty
    ...
    def test_get_user_agent(self):
        with requests_mock.Mocker() as m:
            ...
            assert content == 'awesome_sitemap_parser/{}'.format(__version__)
    ...
    def test_get_nonexistent_domain(self):
        ...
        assert isinstance(response, WebClientErrorResponse)
        assert response.retryable() is False
        assert 'Max retries exceeded with url' in response.message()
